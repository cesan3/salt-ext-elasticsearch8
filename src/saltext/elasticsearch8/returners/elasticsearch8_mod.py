"""
Salt returner module, copied from the original elasticsearch_returner.py writtent by Jurnell Cockhren

Return data to an elasticsearch server for indexing.

:author:        Jurnell Cockhren <jurnell.cockhren@sophicware.com>, Arnold Bechtoldt <mail@arnoldbechtoldt.com>
...
def _ensure_index(index, hosts=None, profile=None):
    # pylint: disable=missing-function-docstring
    index_exists = __salt__["elasticsearch.index_exists"](index, hosts=hosts, profile=profile)
    if not index_exists:
        ...
        __salt__["elasticsearch.index_create"](
            f"{index}-v1", settings=index_definition, hosts=hosts, profile=profile
        )
        __salt__["elasticsearch.alias_create"](
            f"{index}-v1", index, hosts=hosts, profile=profile
        )
...
def returner(ret):
    ...
    if options["index_date"]:
        index = f"{index}-{datetime.date.today().strftime('%Y.%m.%d')}"
    ...
    if job_fun in STATE_FUNCTIONS:
        ...
        if options["states_order_output"] and isinstance(ret["return"], dict):
            ...
            for uid, data in ret["return"].items():
                ...
                new_uid = f"{str(data['__run_num__']).zfill(max_chars)}_|-{uid}"
...
def event_return(events):
    ...
    doc_type = options["master_event_doc_type"]  # pylint: disable=unused-variable
...
def save_load(jid, load, minions=None):  # pylint: disable=unused-argument
    ...
    doc_type = options["master_job_cache_doc_type"]  # pylint: disable=unused-variable
